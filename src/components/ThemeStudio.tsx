...
export function ThemeStudio({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const contrast = (background: string) => {
      const values = [luminance(accent), luminance(background)].sort((a, b) => b - a);
      return (values[0] + .05) / (values[1] + .05);
    };
    const textAccent = [palette.tokens.paper, palette.tokens.surface, palette.tokens.soft, palette.tokens.sage].every(background => contrast(background) >= 4.5) ? accent : palette.tokens.ink;
    document.documentElement.style.setProperty('--accent-text', textAccent);
    if (appearance.accent) { document.documentElement.style.setProperty('--accent', appearance.accent); document.documentElement.style.setProperty('--accent-ink', luminance(appearance.accent) > .179 ? '#000000' : '#ffffff'); }
  ...
  return <dialog ref={dialog} className="theme-dialog" aria-labelledby="theme-title" onCancel={onClose} onClose={onClose} onClick={e => { if (e.target === dialog.current) onClose(); }}><div className="theme-inner">
    ...
    <div className="theme-bottom"><small>Saved only in this browser.</small><button onClick={() => setAppearance(defaults)}><RotateCcw size={14} /> Reset</button></div>
  </div></dialog>;
}
